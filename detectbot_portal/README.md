...
- `pages/07_option_generator.py`: Detect Bot 옵션 생성 UI
...
| `DETECTBOT_MAX_UPLOAD_SIZE_MB` | 리포트 업로드 및 저장 이력 화면의 JSON 업로드 허용 크기를 MB 단위로 지정합니다. | 설정 YAML의 `app.max_upload_size_mb`보다 우선하며, 업로드 저장 전에 파일 크기를 검사합니다. | `set DETECTBOT_MAX_UPLOAD_SIZE_MB=10` | 기본값은 `10`입니다. Settings 화면에서도 조절할 수 있습니다. |
...
리포트 업로드 화면은 두 단계로 파일 크기를 제한합니다.

- Streamlit 위젯 상한: 리포트 업로드 화면의 `st.file_uploader`가 `app.max_upload_size_mb` 값을 받아 파일 선택 UI의 `Limit ... per file` 문구와 브라우저 업로드 상한을 결정합니다.
- 포털 앱 검증: 같은 `app.max_upload_size_mb`, `DETECTBOT_MAX_UPLOAD_SIZE_MB`, Settings 화면의 `Max Report Upload Size (MB)` 값이 업로드 저장 전 검증에도 사용됩니다.

기본 fallback 값은 `.streamlit/config.toml`의 `server.maxUploadSize = 10`과 포털 설정의 `app.max_upload_size_mb: 10`입니다. Settings 화면에서 값을 변경하면 페이지 rerun 후 파일 선택 UI의 제한 문구도 해당 값으로 표시됩니다.
